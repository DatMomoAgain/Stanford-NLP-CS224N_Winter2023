...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    dotProduct = outsideVectors @ centerWordVec    
    predProb = softmax(dotProduct)
    actualProb = np.zeros(predProb.shape)
    actualProb[outsideWordIdx] = 1

    loss = -np.log(predProb[outsideWordIdx])
    gradCenterVec = outsideVectors.T @ (-actualProb + predProb)

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    for i in range(len(gradOutsideVecs)):
        gradOutsideVecs[i] = centerWordVec * (predProb[i] - actualProb[i])
...
def negSamplingLossAndGradient(
    ...
    dotProduct = outsideVectors @ centerWordVec
    loss = -np.log(sigmoid(dotProduct[outsideWordIdx])) - np.sum(np.log(sigmoid(-dotProduct[negSampleWordIndices])))
    gradCenterVec = outsideVectors[outsideWordIdx] * (-1 + sigmoid(dotProduct[outsideWordIdx]))
    for i in negSampleWordIndices:
        gradCenterVec += outsideVectors[i] * (1 - sigmoid(-dotProduct[i]))

    Kones = np.ones(1 + K)
    reuse = Kones - sigmoid(-dotProduct[indices])
    reuse[0] = sigmoid(dotProduct[outsideWordIdx]) - Kones[0]

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    for i in range(len(indices)):
        gradOutsideVecs[indices[i]] += reuse[i] * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centreWordIdx = word2Ind[currentCenterWord]
    outsideWordIdx = [word2Ind[i] for i in outsideWords]

    for i in outsideWordIdx:
        tloss, tgradCenterVec, tgradOutsideVecs = word2vecLossAndGradient(centerWordVectors[centreWordIdx], 
                                                                          i, 
                                                                          outsideVectors,
                                                                          dataset
                                                                          )
        loss += tloss
        gradCenterVecs[centreWordIdx] += tgradCenterVec
        gradOutsideVectors += tgradOutsideVecs
    ### END YOUR CODE
